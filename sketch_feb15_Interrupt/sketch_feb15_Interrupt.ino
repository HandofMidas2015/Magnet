...
//pressure sensor
int presspin=2;
int press_old=0, press_val=0;
int pres=0;
//acceleration and gyro
float ax, ay, az;
float gx, gy, gz;
//for carlibration
float axoffs,ayoffs,azoffs;
float gxoffs,gyoffs,gzoffs;

unsigned long pretime=10, nexttime=10;
...
void setup()
{  
  pinMode(presspin, INPUT);//pressure sensor
  pinMode(1, OUTPUT);//TX
...
void loop()
{
  ...
  {
    digitalWrite(1, 1);
    //Serial.print(nowtime-pretime);
    //pretime=nowtime;
    nexttime=nowtime+10;
    
    
    
    //get values from MPU9150
...
//  Serial.print("  ");
    ...
    //get the values from pressure sensor
    press_val=analogRead(presspin);
    if(press_old != press_val)
    {
      if(press_val > 20 )
      {
        if((ay+ayoffs)<0){
    ...
    Serial.print(",");
    Serial.print("ax=");
    Serial.print((ax+axoffs)/16384);
    Serial.print(",");
    Serial.print("ay=");
    Serial.print((ay+ayoffs)/16384);
    Serial.print(",");
    Serial.print("az=");
    Serial.println((az+azoffs)/16384);
